...
from engines.realtime_data_fabric import build_default_fabric, utc_now
    ...
    for source in fabric.sources():
        try:
            ...
            if result.get("status") != "fetched":
                continue
            observation_result = fabric.ingest_payload(
                source.source_id,
                record_id=result["payload_hash"],
                payload=result["data"],
                observed_at=fabric.state(source.source_id).last_success_at or utc_now(),
                source_version=result.get("etag") or result.get("last_modified"),
                source_uri=source.endpoint,
                etag=result.get("etag"),
                last_modified=result.get("last_modified"),
            )
            if observation_result.accepted:
                stored = next(item for item in fabric.observations() if item.payload_hash == observation_result.payload_hash)
