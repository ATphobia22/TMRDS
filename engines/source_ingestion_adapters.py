...
class ClinicalTrialsEvidenceAdapter:
    ...
    async def ingest(self, response: dict[str, Any]) -> int:
        ...
        for study in response.get("studies", []):
            ...
            await self.ingestion.ingest_observation(source_id="clinicaltrials_gov", source_record_id=source_record_id, payload=study)
            ...
            for intervention in interventions:
                ...
                if not name:
                    continue
                entity_type = EntityType.DRUG if str(intervention.get("type", "")).upper() in {"DRUG", "BIOLOGICAL"} else EntityType.COMPOUND
