...
def build_default_fabric() -> DataFabric:
    ...
    return DataFabric([
        SourceDefinition(source_id="usgs_water", provider="USGS", name="USGS Water Services Instantaneous Values", endpoint="https://waterservices.usgs.gov/nwis/iv/", format="json", update_frequency="real-time", default_params={"format": "json", "stateCd": "IN", "parameterCd": "00060", "siteStatus": "all"}),
        SourceDefinition(source_id="usgs_earthquakes", provider="USGS", name="USGS Earthquake Hazards Program GeoJSON Feed", endpoint="https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_hour.geojson", format="geojson", update_frequency="hourly"),
        SourceDefinition(source_id="noaa_weather", provider="NOAA/NWS", name="National Weather Service API", endpoint="https://api.weather.gov/points/37.9748,-87.5558", format="json", update_frequency="source-defined"),
        SourceDefinition(source_id="nws_alerts", provider="NOAA/NWS", name="National Weather Service Active Alerts", endpoint="https://api.weather.gov/alerts/active", format="geojson", update_frequency="near-real-time", default_params={"status": "actual"}),
        SourceDefinition(source_id="fema_disasters", provider="FEMA", name="FEMA Disaster Declarations Summaries", endpoint="https://www.fema.gov/api/open/v2/DisasterDeclarationsSummaries", format="json", update_frequency="source-defined", default_params={"$top": "1000"}),
        SourceDefinition(source_id="cdc_cdi", provider="CDC", name="CDC Chronic Disease Indicators", endpoint="https://data.cdc.gov/resource/a8ys-9fjs.json", format="json", update_frequency="source-defined", default_params={"$limit": "500"}),
        SourceDefinition(source_id="clinicaltrials_gov", provider="NIH/NLM", name="ClinicalTrials.gov API v2", endpoint="https://clinicaltrials.gov/api/v2/studies", format="json", update_frequency="daily", default_params={"pageSize": "100"}),
        SourceDefinition(source_id="openfda", provider="FDA", name="openFDA drug labels", endpoint="https://api.fda.gov/drug/label.json", format="json", update_frequency="source-defined", default_params={"limit": "100"}),
        SourceDefinition(source_id="pubmed", provider="NCBI/NLM", name="PubMed E-utilities", endpoint="https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi", format="json", update_frequency="source-defined", default_params={"db": "pubmed", "retmode": "json", "term": "*", "retmax": "100"}),
        SourceDefinition(source_id="europe_pmc", provider="EMBL-EBI", name="Europe PMC Search API", endpoint="https://www.ebi.ac.uk/europepmc/webservices/rest/search", format="json", update_frequency="source-defined", default_params={"format": "json", "query": "OPEN_ACCESS:Y", "pageSize": "100"}),
        SourceDefinition(source_id="nlm_clinical_tables", provider="NLM", name="NLM Clinical Tables Conditions", endpoint="https://clinicaltables.nlm.nih.gov/api/conditions/v3/search", format="json", update_frequency="source-defined", default_params={"terms": "diabetes", "maxList": "50"}),
    ])
